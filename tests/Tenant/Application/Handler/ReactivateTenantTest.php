...
use App\Application\Tenant\Handler\ReactivateTenant;
...
{
    public function test_reactivates_suspended_tenant(): void
    {
        ...
        $persister
            ...
            ->with($this->callback(function ($savedTenant) {
                return $savedTenant->getStatus() === TenantStatus::ACTIVE
                    && $savedTenant->getSuspendedAt() === null;
            }));

        $handler = new ReactivateTenant($provider, $persister);
        $command = new ReactivateTenantCommand(tenantId: $tenantId->toString());
