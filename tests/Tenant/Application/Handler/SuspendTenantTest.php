...
use App\Application\Tenant\Handler\SuspendTenant;
...
{
    public function test_suspends_active_tenant(): void
    {
        ...
        $persister
            ...
            ->with($this->callback(function ($savedTenant) {
                return $savedTenant->getStatus() === TenantStatus::SUSPENDED
                    && $savedTenant->getSuspendedAt() !== null;
            }));

        $handler = new SuspendTenant($provider, $persister);
        $command = new SuspendTenantCommand(tenantId: $tenantId->toString());
